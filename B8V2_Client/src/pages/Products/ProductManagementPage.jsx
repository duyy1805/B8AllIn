...
import { getDocumentTypeDefaultAudience, getDocumentTypes, getProductCustomers } from '../../api/master.api';
import {
  ...
  removeProductDocumentAudience, deleteProductTrainingEvidence, bulkAssignProductCustomer
...
function ProductMasterWorkspace() {
  ...
  const [wizardLockedTypeId, setWizardLockedTypeId] = useState(null);
  ...
  const wizardTypeId = Form.useWatch('documentTypeId', wizardForm);
  ...
  const productOptions = useQuery({
    ...
    enabled: ['wizard', 'requirements', 'customer'].includes(modal)
  });
  ...
  const openWizard = (contextProductId, documentTypeId) => {
    setWizardContextProductId(contextProductId || null);
    setWizardLockedTypeId(documentTypeId || null);
    setItemCodeSearch('');
    setWizardFileList([]);
    wizardForm.resetFields();
    wizardForm.setFieldsValue({ issueDate: dayjs(), effectiveDate: dayjs(), additionalProductIds: [], documentTypeId });
    setModal('wizard');
  };

  useEffect(() => {
    if (modal !== 'wizard' || !wizardTypeId || !defaultAudience.data) return;
    wizardForm.setFieldsValue({
      departmentIds: [...new Set(defaultAudience.data.map(item => Number(item.DepartmentId)))]
    });
  }, [defaultAudience.data, modal, wizardForm, wizardTypeId]);
  ...
  const customerMutation = useMutation({
    mutationFn: bulkAssignProductCustomer,
    onSuccess: data => {
      message.success(`Đã cập nhật khách hàng cho ${data.UpdatedCount} ItemCode`);
      setModal(null); customerForm.resetFields(); setSelectedIds([]); refresh();
    },
    onError: error => message.error(error.response?.data?.message || error.message)
  });
  ...
  return <div className={`process-workspace product-workspace ${selectedId ? 'has-drawer' : ''}`}>
    ...
    <Modal title="Cấu hình loại tài liệu" open={modal === 'requirements'} onCancel={() => setModal(null)} onOk={() => requirementForm.submit()} confirmLoading={requirementMutation.isPending}>
      <Form form={requirementForm} layout="vertical" onFinish={requirementMutation.mutate}>
        ...
        <Form.Item name="action" label="Thao tác" rules={[{ required: true }]}><Select options={[{ value: 'ADD', label: 'Thêm loại tài liệu' }, { value: 'REMOVE', label: 'Gỡ loại tài liệu' }]} /></Form.Item>
    ...
    <Modal title="Gán khách hàng cho ItemCode" open={modal === 'customer'} onCancel={() => setModal(null)} onOk={() => customerForm.submit()} confirmLoading={customerMutation.isPending}>
      <Form form={customerForm} layout="vertical" onFinish={customerMutation.mutate}>
        <Form.Item name="productIds" label="ItemCode áp dụng" rules={[{ required: true, message: 'Hãy chọn ít nhất một ItemCode' }]}><Select mode="multiple" showSearch filterOption={false} onSearch={setItemCodeSearch} loading={productOptions.isFetching} options={(productOptions.data || []).map(item => ({ value: item.Id, label: `${item.ItemCode} · ${item.ProductName || ''}` }))} /></Form.Item>
        <Form.Item name="customerCode" label="Khách hàng"><Select allowClear options={(customers.data || []).map(item => ({ value: item.Code, label: item.Name }))} placeholder="Để trống nếu chưa phân loại" /></Form.Item>
      </Form>
    </Modal>

    <Modal width={720} title={wizardContextProductId ? `Thêm tài liệu · ${product?.ItemCode || ''}` : 'Thêm tài liệu đầu tiên'} open={modal === 'wizard'} onCancel={() => { setModal(null); setWizardFileList([]); setWizardContextProductId(null); setWizardLockedTypeId(null); }} onOk={() => wizardForm.submit()} confirmLoading={wizardMutation.isPending} okText={wizardFileList.length ? 'Lưu và phát hành' : 'Lưu bản nháp'}>
      <Form form={wizardForm} layout="vertical" onFinish={wizardMutation.mutate}>
        ...
        <Form.Item name="documentTypeId" label="Loại tài liệu" rules={[{ required: true }]} extra={wizardLockedTypeId ? 'Loại tài liệu được chọn từ danh sách bắt buộc và không thể thay đổi trong lần thêm này.' : null}><Select disabled={Boolean(wizardLockedTypeId)} showSearch optionFilterProp="label" placeholder="Nhập tên loại tài liệu để tìm" options={wizardTypes.map(item => ({ value: item.Id, label: item.Name }))} /></Form.Item>
        ...
        <Form.Item name="departmentIds" label="Bộ phận nhận" extra="Đã chọn sẵn từ quy trình sản xuất; có thể thêm hoặc bỏ trước khi lưu."><DepartmentSelect mode="multiple" /></Form.Item>
...
export default function ProductManagementPage() {
  const {hasPermission}=useAuth();
  const canManage=['DOCUMENT_VIEW_ALL','DOCUMENT_CREATE','DOCUMENT_VERSION_CREATE','DOCUMENT_FILE_UPLOAD','DOCUMENT_AUDIENCE_MANAGE','PRODUCT_SYNC','PRODUCT_REQUIREMENT_MANAGE','PRODUCT_CUSTOMER_ASSIGN','PRODUCT_MANAGE','PRODUCT_EDIT','PRODUCT_DELETE','PRODUCT_DOCUMENT_EDIT','PRODUCT_DOCUMENT_DELETE','PRODUCT_DOCUMENT_VERSION_EDIT','PRODUCT_DOCUMENT_VERSION_DELETE'].some(permission=>hasPermission(permission));
